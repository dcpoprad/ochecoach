...
const dict = {
    EN: { welcomeTitle: "WELCOME TO OCHE COACH", welcomeText: "This is your pure, zen training tool. No counting, no scoring, no pressure. Just you, the board, and the time flowing.", btnUnderstand: "GOT IT", duration: "DURATION", focus: "FOCUS", tournament: "TOURNAMENT", btnGoToOche: "GO TO OCHE", back: "BACK", exit: "EXIT", btnChange: "CHANGE", btnGameOn: "GAME ON!", btnPause: "|| PAUSE", btnResume: "► RESUME", btnNextBlock: "NEXT BLOCK", coachTip: "Take a short break between blocks. Switch off your focus, hydrate, and rest your arm. Matches take about 10-20 minutes, so it's important to practice resting and restarting your concentration." },
    SK: { welcomeTitle: "VITAJ V OCHE COACH", welcomeText: "Toto je tvoj tréner na čiare. Bez zapisovania, bez hodnotenia, žiaden tlak. Len ty, šípky, terč a plynúci čas.", btnUnderstand: "GOT IT", duration: "DURATION", focus: "FOCUS", tournament: "TOURNAMENT", btnGoToOche: "GO TO OCHE", back: "BACK", exit: "EXIT", btnChange: "CHANGE", btnGameOn: "GAME ON!", btnPause: "|| PAUSE", btnResume: "► RESUME", btnNextBlock: "NEXT BLOCK", coachTip: "Medzi blokmi si daj krátku pauzu. Vypni sústredenie, napi sa a zves ruky. Zápasy trvajú cca 10-20 minút, preto je dôležité trénovať aj oddych a reštart koncentrácie." }
};
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.time-tile').forEach(tile => {
        tile.addEventListener('click', function() {
            ...
            else mixTile.classList.remove('disabled');
            checkRequirements();
        });
    });
    ...
    document.querySelectorAll('.focus-tile').forEach(tile => {
        tile.addEventListener('click', function() {
            ...
            document.getElementById('warmUpBtn').classList.remove('active'); document.querySelectorAll('.focus-tile').forEach(f => f.classList.remove('active')); this.classList.add('active'); selectedFocus = this.getAttribute('data-val'); checkRequirements();
        });
    });
    ...
    document.getElementById('enterBtn').addEventListener('click', () => {
        ...
        document.getElementById('lobbyView').classList.add('hidden'); document.getElementById('hintView').classList.remove('hidden');
        document.getElementById('langToggle').classList.add('hidden'); document.getElementById('infoToggle').classList.add('hidden');
    });
    ...
    document.getElementById('hintReadyBtn').addEventListener('click', () => {
        ...
        document.getElementById('hintView').classList.add('hidden'); document.getElementById('timerView').classList.remove('hidden');
    ...
    document.getElementById('mainBtn').addEventListener('click', () => {
        if (currentState === 'RUNNING') { clearInterval(timerId); currentState = 'PAUSED'; updateTimerUI(); } 
            ...
            currentBlockIndex++;
            if (currentBlockIndex >= currentBlockPlan.length) document.querySelector('#timerView .exit-trigger').click();
            else {
                currentGameIndex = Math.floor(Math.random() * currentActiveGames.length); updateHintTexts();
                document.getElementById('timerView').classList.add('hidden'); document.getElementById('hintView').classList.remove('hidden');
                document.getElementById('timerDisplay').classList.remove('hidden');
                if(document.getElementById('coachTipBox')) document.getElementById('coachTipBox').classList.add('hidden');
                if(document.getElementById('completionRing')) { document.getElementById('completionRing').classList.add('hidden'); document.getElementById('completionRing').classList.remove('ring-animate'); }
            }
    ...
    document.querySelectorAll('.exit-trigger').forEach(trigger => trigger.addEventListener('click', () => {
...
function startTimer() {
    ...
    timerId = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            setTimeout(() => { if (selectedFocus === 'Tournament') document.querySelector('#timerView .exit-trigger').click(); else { currentState = 'FINISHED'; updateTimerUI(); } }, 3000);
        } else updateTimerUI();
    }, 1000);
    updateTimerUI();
}
...
function updateTimerUI() {
    ...
    if (!disp) return;
    let t = TIME_MULTIPLIER !== 1 && currentState !== 'FINISHED' ? timeLeft * 60 : timeLeft;
